 ...
 * Version:  V_1.1                                                             *
 *                                                                             *
 * Created on April 16, 2021, 12:56 PM                                         *
 * *****************************************************************************/
...
void WS2812FX::service()
{
  if (_running || _triggered)
  {
    ...
    if (doShow)
    {
      vTaskDelay(1); //delay(1); // for ESP32 (see https://forums.adafruit.com/viewtopic.php?f=47&t=117327)
      show();
    }
    _triggered = false;
  }
...
uint32_t *WS2812FX::intensitySums()
{
  ...
  for (uint16_t i = 0; i < numBytes; i += bytesPerPixel)
  {
    ...
    if (bytesPerPixel == 4)
      intensities[3] += pixels[i + 3]; // for RGBW LEDs
  }
  return intensities;
}
...
uint16_t WS2812FX::scan(uint32_t color1, uint32_t color2, bool dual)
{
  ...
  if (_seg_rt->counter_mode_step >= (uint16_t)(_seg_len - size))
    _seg_rt->aux_param = 1;
...
uint16_t WS2812FX::chase(uint32_t color1, uint32_t color2, uint32_t color3)
{
  ...
  if (_seg_rt->counter_mode_step + (size * 3) == _seg_len)
    SET_CYCLE;
