...
int MPU9250::begin()
{
  ...
  vTaskDelay(100); // delay(100); // long wait between AK8963 mode changes
...
// enableWakeOnMotion()
// @param womThresh_mg: float
// @param odr: LpAccelOdr
// @return state: int
...
int MPU9250::readAK8963Registers(uint8_t subAddress, uint8_t count, uint8_t *dest)
{
  ...
  vTaskDelay(1); // delay(1); // takes some time for these registers to fill
